...
#define RAYON_ATTRACTION 155
#define RAYON_ORIENTATION 70
...
void poisson_deplacer(Poisson* p){
    ...
    Vector2 repulsion_mur = {0, 0};
    float distance_alerte = 5 * TAILLE_POISSON; 
    // Mur gauche
    if(p->pos.x < distance_alerte){
        float intensite = (distance_alerte - p->pos.x) / distance_alerte;
        repulsion_mur.x += intensite;
    }
    // Mur droit
    if(p->pos.x > LARGEUR - distance_alerte){
        float intensite = (p->pos.x - (LARGEUR - distance_alerte)) / distance_alerte;
        repulsion_mur.x -= intensite;
    }
    // Mur haut
    if(p->pos.y < distance_alerte){
        float intensite = (distance_alerte - p->pos.y) / distance_alerte;
        repulsion_mur.y += intensite;
    }
    // Mur bas
    if(p->pos.y > HAUTEUR - distance_alerte){
        float intensite = (p->pos.y - (HAUTEUR - distance_alerte)) / distance_alerte;
        repulsion_mur.y -= intensite;
    }
    
    // Application de la répulsion si nécessaire
    float norme_repulsion = sqrtf(repulsion_mur.x * repulsion_mur.x + repulsion_mur.y * repulsion_mur.y);
    if(norme_repulsion > 0){
        repulsion_mur.x /= norme_repulsion;
        repulsion_mur.y /= norme_repulsion;
        
        float facteur = WALL; // Utilise ton paramètre WALL existant
        p->vitesse.x = (1-facteur)*p->vitesse.x + facteur*repulsion_mur.x*V_INITIALE;
        p->vitesse.y = (1-facteur)*p->vitesse.y + facteur*repulsion_mur.y*V_INITIALE;
    }
    
    p->pos = vec_add(p->pos, p->vitesse);
    
    // Contraintes dures (sécurité)
    if(p->pos.x < TAILLE_POISSON) p->pos.x = TAILLE_POISSON;
    if(p->pos.x > LARGEUR - TAILLE_POISSON) p->pos.x = LARGEUR - TAILLE_POISSON;
    if(p->pos.y < TAILLE_POISSON) p->pos.y = TAILLE_POISSON;
    if(p->pos.y > HAUTEUR - TAILLE_POISSON) p->pos.y = HAUTEUR - TAILLE_POISSON;
}
...
void draw_filled_circle(SDL_Renderer* renderer, int cx, int cy, int r) {
    ...
    // Dessiner les points des 8 octants et remplir le cercle
    while (y >= x) {
...
void poisson_dessiner(Poisson* p, SDL_Renderer* renderer, SDL_Color c1, SDL_Color c2, SDL_Color c3, SDL_Color c4,SDL_Color c5,SDL_Color c6, Poisson voisin[NB_POISSONS]){
    ...
    draw_filled_circle(renderer,(int)p->pos.x,(int)p->pos.y,TAILLE_POISSON);
    ...
    draw_filled_circle(renderer,(int)(p->pos.x + 3.5/5 * dx),(int)(p->pos.y + 3.5/5 * dy),TAILLE_POISSON);
...
Vector2 moyenne_position(Poisson poisson[NB_POISSONS], SDL_Renderer* renderer, int nb_frame, Vector2 moy_prec, Vector2 tab_moy[3], float* pgroupe, float* mgroupe){
    float mx = 0;
    float my = 0;
    for(int i=0; i<NB_POISSONS; i++){
        mx += poisson[i].pos.x;
    ...
    mx = mx / NB_POISSONS;
    my = my / NB_POISSONS;
    //draw_filled_circle(renderer, (int)mx, (int)my, TAILLE_POISSON);
    
    bool dist_moy = distance((Vector2){mx, my}, moy_prec) < 1;
    ...
    if(cycle){
        float rm = rayon_moyen(poisson, mx, my,renderer);
    ...
    if(nb_frame % 100 == 0){
        ...
        printf("Cycle ? %s\n", cycle ? "Oui" : "Non");
        printf("    -> distance à ancienne moyenne : %.2f \n", distance((Vector2){mx, my}, moy_prec));
        printf("    -> dispersion des poissons : %d \n", (int)(max-min));
        printf("nb_frame : %d", nb_frame);
        printf("\n");
        printf("➫ Pgroup : %.3f\n", pgr);
        printf("➫ Mgroup : %.3f\n", mgr);
        printf("\n \n");
...
int main(){
    ...
    SDL_Window* window = SDL_CreateWindow("MES POISSONS D'AMOUR",SDL_WINDOWPOS_CENTERED,SDL_WINDOWPOS_CENTERED,LARGEUR,HAUTEUR,  SDL_WINDOW_SHOWN  /*SDL_WINDOW_FULLSCREEN */);
    ...
    while(running){
        ...
        Vector2 moy = moyenne_position(poissons, renderer, nb_frame, moy, tab_moy, pgroup, mgroup);   
